...
def relation_OT(feat, P, ratio, reg_1=0.01, reg2=1.,):
    feat = feat - feat.mean(0, keepdim=True)
    feat = F.normalize(feat, dim=1, p=2)
    P = F.log_softmax(P, dim=-1)
    # import pdb
    # pdb.set_trace()
    couplings, selected_mask = curriculum_structure_aware_PL(feat, P, top_percent=ratio, reg_feat=5.0, reg_1=reg_1, reg2=reg2,)
    # import pdb
    # pdb.set_trace()
    row_sum = torch.sum(couplings, 1).reshape((-1,1)) + 1e-6
    ...
    return argmax_plabels, conf, selected_mask
...
def entropic_COT_gcg(a, b, M, reg1, reg2, f, df, G0=None, numItermax=10,
    ...
    def cost(G):
        ...
        safe_G = G + 1e-16
        if reg1 > 1e-10:
            entropy_term = reg1 * nx.sum(safe_G * nx.log(safe_G))
            cost_val += entropy_term
        if reg2 > 1e-10:
            structure_term = reg2 * f(G)
            cost_val += structure_term
    ...
    while loop:
        ...
        # line search
        if reg1 > 1e-10:
            dcost = Mi + reg1 * (1 + nx.log(G + 1e-16))  #
        else:
            dcost = Mi         
...
def entropic_COT(a, b, M, reg, numItermax=1000,
    ...
    reg = max(reg, 1e-16)
    scaled_M = M / (-reg)
    scaled_M = torch.clamp(scaled_M, min=-100, max=100)
    K = torch.exp(scaled_M)
...
def curriculum_structure_aware_PL(features, P, top_percent, L=None,
                                    reg_feat=2., reg_lab=2., temp=1, device=None, version='normal', reg_1=0.01, reg2=1., reg_sparsity=None):
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    def structure_aware_f(Q):
        if L is None:
            res = reg_feat * omega(S, torch.ones_like(P), Q)
        else:
            res = reg_feat * omega(S, torch.ones_like(P), Q) + reg_lab * omega(S, L, Q)
        return -res

    def structure_aware_df(Q):
        if L is None:
            gradient = reg_feat * omega_df(S, torch.ones_like(P), Q)
        else:
            gradient = reg_feat * omega_df(S, torch.ones_like(P), Q) + reg_lab * omega_df(S, L, Q)
        return -gradient
    ...
    coupling = entropic_COT_extra_reg(a, b, M=-P, reg1=reg_1, reg2=reg2, # ablation study
