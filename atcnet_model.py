...
class ATCNetModule(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.atc_blocks):
            out = blk(x[:, i:(seq_len-self.n_windows+i+1), :])
            blk_output = blk_output + out[1]
            blk_feats = blk_feats + out[0]

        blk_output = blk_output / self.n_windows
        blk_feats = blk_feats / self.n_windows
        if not self.training:
            return blk_feats, blk_output
        return blk_output