...
class ExperimentProcessor:
    def __init__(self, subject_number):
        ...
        if self.dataset == '2a':
            ...
            self.class_num = 2
            self.channel_num = 3
            self.root = './datasets/2b/'
        elif self.dataset == 'hgd':
            self.class_num = 4
            self.channel_num = 3
            self.root = './datasets/hgd/'
        self.net = ATCNetModule(self.class_num, self.channel_num).cuda()
    ...
    def get_train_test_data(self):
        ...
        train_data = []
        train_label = []
        # total_data = scipy.io.loadmat(self.root + '1.mat')
        for i in range(1, 10):
            if i == self.subject_number:
                continue
            total_data = scipy.io.loadmat(self.root + 'A0%dT.mat' % i)
            train_data.append(total_data['data'])
            train_label.append(total_data['label'])
        train_data = np.concatenate(train_data, axis=0)
        train_label = np.concatenate(train_label, axis=0)
        print(train_data.shape)
        if self.dataset == '2b' or self.dataset == '2a2': 
        ...
        # test data
        test_tmp = scipy.io.loadmat(self.root + 'A0%dE.mat' % self.subject_number)
    ...
    def train(self):
        ...
        for epoch in range(self.epoch_number):
            ...
            train_acc = 0
            train_loss = 0
            for i, (t_data, t_label) in enumerate(train_loader):
                outputs = self.net(t_data)
                loss = self.criterion_cls(outputs, t_label)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            for i, (t_data, t_label) in enumerate(test_loader):
                ...
                test_loss += self.criterion_cls(outputs, t_label).item()
                # import pdb
                # pdb.set_trace()
                t_predict, conf, _ = relation_OT(feats.detach(), outputs.detach()/10, ratio=1.0)
                # t_predict = torch.max(outputs, 1)[1]
            ...
            print("subject: %d, epoch: %d, train loss: %.5f, train accuracy: %.3f,"
                 " test loss is %.5f, test accuracy is %.3f" % (self.subject_number, epoch, train_loss, train_acc,
                                                                test_loss,
                                                                test_acc))
            average_accuracy += test_acc
            if test_acc > best_accuracy:
                best_accuracy = test_acc
                torch.save(self.net.state_dict(), './ckpts/{}_{}_best.pth'.format(self.dataset,self.subject_number))
